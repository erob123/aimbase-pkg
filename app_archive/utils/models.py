from instarest.db import base
from instarest.aimodels.bertopic import schemas
...
def dict_optional_column_attrs(
    ModelType: EntityModel,
...
def gen_relationship_attrs(
    ModelType: EntityModel, schema_list: list[object] = []
) -> Generator[tuple[str, tuple[type | Any, ...]], None, None]:
    """
    Generator for pulling attributes and types from the declared model.
    Includes foreign keys if actually saved in the table, but does not
...
for field_name, (field_type, _) in gen_column_attrs(EntityModel):
    print(field_name, field_type)


for field_name, (field_type, _) in gen_relationship_attrs(EntityModel, ins.getmembers(schemas)):
    print(field_name, field_type)
